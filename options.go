...
func WithContainerName(containerNameParts ...string) TestContainerOption {
	option := func(options *testContainerOptions) (err error) {
		// remove empty parts
		containerNameParts = slices.DeleteFunc(containerNameParts, func(s string) bool { return s == "" })

		// validate
		if len(containerNameParts) == 0 {
			return fmt.Errorf("%w: containerNameParts must not be empty", ErrOptionInvalid)
		}

		// replace invalid chars
		containerName := strings.Join(containerNameParts, "-")

		// set option
		options.containerName = containerNameInvalidCharsRegexp.ReplaceAllString(containerName, "-")
	...
	optionWrap := func(options *testContainerOptions) (err error) {
		err = option(options)
		if err != nil {
			return fmt.Errorf("failed to apply WithContainerName with value `%#+v`: %w", containerNameParts, err)
		}
